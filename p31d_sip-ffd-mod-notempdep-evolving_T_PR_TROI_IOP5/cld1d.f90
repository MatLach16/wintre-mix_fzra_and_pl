...
!------------------------------------------------------------------------------!
    ...
    ! Add constant for dew point
      ...
      real, dimension(nk_input)    :: z,p,tt,rh,rhi,td,w1,DIV,Tdry,zcld,w1cld,rho,alfa2
      real, dimension(nk_input)    :: z_1,p_1,tt_1,td_1,qv_1,rh_1,rhi_1
      real, dimension(nk_input)    :: z_2,p_2,tt_2,td_2,qv_2,rh_2,rhi_2
      real, dimension(nk_input)    :: z_3,p_3,tt_3,td_3,qv_3,rh_3,rhi_3
      real, dimension(nk_input)    :: z_4,p_4,tt_4,td_4,qv_4,rh_4,rhi_4
      real, dimension(nk_input)    :: z_5,p_5,tt_5,td_5,qv_5,rh_5,rhi_5
      ...
      real :: time_sounding1,time_sounding2,time_sounding3,time_sounding4,time_sounding5
    ...
    ! Diagnostics, etc.
      ...
      real                     :: qitop_csv,nitop_csv,esw,esi
!---------------------------------------------------------------------------------------!
    ! ****** output files *****
...
!------- INITIALIZE w, tt, td, p AND Q[x]  PROFILES: -------
      ...
      ! Sounging #1
      time_sounding1 = 3600.+10*60  ! Time in seconds 
      open(unit=12, file='/home/rml000/sitestore8/UQAM/projet_margaux/Pour_Mathieu_fig4/' &
      //'create_figure_papier/simu1d/src_sip-ffd-mod-notempdep-evolving_T_PR_TROI_IOP5/temp_to_use_0.dat')
      read(12,*) nlvs
      read(12,*)
      read(12,*)
      read(12,*)
    ...
    ! Sounging #2
      time_sounding2 = 3600.*3. +10*60 ! Time in seconds
      open(unit=13, file='/home/rml000/sitestore8/UQAM/projet_margaux/Pour_Mathieu_fig4/' &
      //'create_figure_papier/simu1d/src_sip-ffd-mod-notempdep-evolving_T_PR_TROI_IOP5/temp_to_use_1.dat')
      read(13,*) nlvs
      read(13,*)
      read(13,*)
      read(13,*)
    ...
    ! Sounging #3
      time_sounding3 = 3600.*7. +10*60  ! Time in seconds
      open(unit=14, file='/home/rml000/sitestore8/UQAM/projet_margaux/Pour_Mathieu_fig4/' &
      //'create_figure_papier/simu1d/src_sip-ffd-mod-notempdep-evolving_T_PR_TROI_IOP5/temp_to_use_2.dat')
      read(14,*) nlvs
      read(14,*)
      read(14,*)
      read(14,*)
      ...
      //'create_figure_papier/simu1d/src_sip-ffd-mod-notempdep-evolving_T_PR_TROI_IOP5/df_manobs_iop5_trois_1s.csv')
      read(17,*)
...
!  Using only the first sounding for now (Zin1, Pin1, TTin1, QVin1)
     ...
     GZ(1,:)= z*GRAV
      ...
      !    esw      = polysvp1(tt(k),0)
      !    esi      = polysvp1(tt(k),1)
      !    rhi_1(k) = rh_1(k) * esw/esi
      !    rhi_2(k) = rh_2(k) * esw/esi
      !    rhi_3(k) = rh_3(k) * esw/esi
...
!  MAIN TIME LOOP:
      ...
      do step = 1,nt
        ...
        tmin = nint(tminr)     !integration time [min]  (integer)
            ...
            if (tsec .lt. time_sounding1) then
            do k = 1,nk_input
                  tt(k) = tt_1(k) 
                  rh(k) = rh_1(k) 
                  ! rhi(k) = rhi_1(k) + (rhi_2(k)-rhi_1(k))*(tsec-time_sounding1)/(time_sounding2-time_sounding1)
                  ! Qv1(1,k) = qv_1(k) + (qv_2(k)-qv_1(k))*(tsec-time_sounding1)/(time_sounding2-time_sounding1)
            enddo
            elseif ((tsec .ge. time_sounding1) .and. (tsec .lt. time_sounding2)) then
            do k = 1,nk_input
            ...
            do k = 1,nk_input
                  ...
                  ! rhi(k) = rhi_2(k) + (rhi_3(k)-rhi_2(k))*(tsec-time_sounding2)/(time_sounding3-time_sounding2)
                  ! Qv1(1,k) = qv_2(k) + (qv_3(k)-qv_2(k))*(tsec-time_sounding2)/(time_sounding3-time_sounding2)
            enddo
            endif

            ! do k = 1,nk_input
            ! if (tsec .lt. (time_sounding2/2.) ) then
            !       rh(k) = rh_1(k)
            ! elseif ((tsec .ge. (time_sounding2/2.)) .and. (tsec .lt. (time_sounding2+time_sounding3)/2.)) then
            !       rh(k) = rh_2(k)
            ! elseif  (tsec .ge. (time_sounding2+time_sounding3)/2.) then
            !       rh(k) = rh_3(k)
            ! endif
            ! enddo
            ...
            do k = 1,nk_input
                  ...
                  !       Qv0(1,k) = 0.622*rh(k)/100.*esw/p(k)
                  ! end if
                  esw      = polysvp1(tt(k),0)
                  Qv1(1,k) = 0.622*rh(k)/100.*esw/p(k)
                  Qv0(1,k) = 0.622*rh(k)/100.*esw/p(k)
