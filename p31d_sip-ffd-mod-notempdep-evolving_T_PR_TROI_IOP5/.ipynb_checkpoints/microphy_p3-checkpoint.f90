...
!__________________________________________________________________________________________!
!                                                                                          !
! Version:       5.3.12 + bugfix_LTscollection                                             !
...
! Local variables and parameters:
 ...
 character(len=1024), parameter :: version_p3                    = '5.3.13'
 character(len=1024), parameter :: version_intended_table_1_2mom = '6.6-2momI'!'6.9-2momI'
 character(len=1024), parameter :: version_intended_table_1_3mom = '6.6-3momI'!'6.9-3momI'
 character(len=1024), parameter :: version_intended_table_2      = '6.1'!'6.2'
...
! read_path = '/fs/homeu2/eccc/mrd/ords/rpnatm/mec000/p3_lookup_tables/p3v5313_p3v543'
...
!==========================================================================================!
 ...
 SUBROUTINE p3_main(qc,nc,qr,nr,th_old,th,qv_old,qv,dt,qitot,qirim,qiliq,nitot,birim,     &
                    ...
                    diag_vis,diag_vis1,diag_vis2,diag_vis3,diag_dhmax,outfreq)
...
if (nCat.eq.1) then
  write(8000,'(1f35.9,1i5,19f50.9)') it*dt,k,qimlt(1)*10**8,qrmlt(1)*10**8,qrcol(1)*10**8,qccol(1)*10**8, &
  ...
  write(9000,'(1f35.9,1i5,10f35.18)')  it*dt,k,ninuc(1),nimlt(1),nisub(1),&
        nislf(1),nrhetc(1),nrheti(1),nchetc(1),ncheti(1),nimul(1),nlevp(1)
endif

if (nCat.eq.2) then
  write(8000,'(1f35.9,1i5,37f50.9)') it*dt,k,qimlt(1)*10**8,qrmlt(1)*10**8,qrcol(1)*10**8,qccol(1)*10**8, &
        nrshdr(1)*10**8,nrcol(1),qisub(1)*10**8,qidep(1)*10**8, &
        qrcon*10**8,qrevp*10**8,qwgrth(1)*10**8,qinuc(1)*10**8, &
        qcheti(1)*10**8, qchetc(1)*10**8, qrhetc(1)*10**8, qrheti(1)*10**8, &
        qrmul(1)*10**8,qcshd(1)*10**8,nimul(1), &
  ...
  write(9000,'(1f35.9,1i5,21f35.18)')  it*dt,k,ninuc(1),nimlt(1),nisub(1),&
        nislf(1),nrhetc(1),nrheti(1),nchetc(1),ncheti(1),nimul(1),nlevp(1),&
...
if (nCat.eq.3) then
  write(8000,'(1f35.9,1i5,56f50.9)') it*dt,k,qimlt(1)*10**8,qrmlt(1)*10**8,qrcol(1)*10**8,qccol(1)*10**8, &
        nrshdr(1)*10**8,nrcol(1),qisub(1)*10**8,qidep(1)*10**8, &
        qrcon*10**8,qrevp*10**8,qwgrth(1)*10**8,qinuc(1)*10**8, &
        qcheti(1)*10**8, qchetc(1)*10**8, qrhetc(1)*10**8, qrheti(1)*10**8, &
        qrmul(1)*10**8,qcshd(1)*10**8,nimul(1), &
...
if (nCat.eq.4) then
  write(8000,'(1f35.9,1i5,76f50.9)') it*dt,k,qimlt(1)*10**8,qrmlt(1)*10**8,qrcol(1)*10**8,qccol(1)*10**8, &
        nrshdr(1)*10**8,nrcol(1),qisub(1)*10**8,qidep(1)*10**8, &
        qrcon*10**8,qrevp*10**8,qwgrth(1)*10**8,qinuc(1)*10**8, &
        qcheti(1)*10**8, qchetc(1)*10**8, qrhetc(1)*10**8, qrheti(1)*10**8, &
        qrmul(1)*10**8,qcshd(1)*10**8,nimul(1), &
        ...
        qrmul(4)*10**8,qcshd(4)*10**8,nimul(4),qicol(1,4)*10**8,qicol(2,4)*10**8,qicol(3,4)*10**8
...
!  enddo
  ...
  function compute_fragments_per_freezing_drop_lawson_adjusted(D, T, N_val)
    implicit none
    real, intent(in) :: D, T    ! Input: diameter (D) and temperature (T)
    real, intent(out) :: N_val,N_val_noT  ! Output: number of fragments

    N_val_noT = 2.5*10**-11*(D*10**6.)**4.     
    if (T .gt. 270.15) then
        N_val = 0.
    else if  (T .gt. 267.15) then
        N_val = N_val_noT*(T-270.15)
    else
     N_val = N_val_noT
    
    endif
    
  end function compute_fragments_per_freezing_drop_lawson_adjusted

  function compute_fragments_per_freezing_drop_phillips_adjusted(D, T, N_val)
    implicit none
    real, intent(in) :: D, T    ! Input: diameter (D) and temperature (T)
    real, intent(out) :: N_val  ! Output: number of fragments

    real :: D_used, eta_mod, T0_val, xi_val, beta_val, Omega_val
    real :: X, log10D, E_val

    ! Ensure D doesn't exceed the upper limit in the model
    D_used = min(D, 1.6e-3)

    log10D = log10(D_used * 1.0e3)  ! Convert D to mm and compute log10
    ...
    if (D < 0.4e-3) then
        ...
        beta_val = -0.1839 * log10D**2 - 0.2017 * log10D - 0.0512
    end if

    ! xi(D)
    xi_val = 10 ** (2.4268 * log10D**3 + 3.3274 * log10D**2 + &
                       2.0783 * log10D + 1.2927)

    ! eta_mod(D, T)
    if (D > 500.e-6) then
        eta_mod = 10 ** log10(6.)
    else if (D > 75.7e-6) then
        eta_mod = 10 ** log10(3.6155 + (6. - 3.6155) * (D - 75.7e-6) / &
                                 (500.e-6 - 75.7e-6))
    else
        eta_mod = 10 ** (0.1242 * log10D**3 - 0.2316 * log10D**2 &
                            - 0.9874 * log10D - 0.0827)
    end if

    ! T0(D)
    T0_val = -1.3999 * log10D**3 - 5.3285 * log10D**2 - &
              3.9847 * log10D - 15.0332
    ...
    ! Compute N_val
    if (D > 1.6e-3) then
        N_val = E_val * Omega_val * (xi_val * eta_mod**2 / ((T - T0_val)**2 + eta_mod**2) + &
                                      beta_val * T)
        N_val = N_val * D / D_used
    else
        N_val = E_val * Omega_val * (xi_val * eta_mod**2 / ((T - T0_val)**2 + eta_mod**2) + &
                                      beta_val * T)
    end if
  end function compute_fragments_per_freezing_drop_phillips_adjusted
