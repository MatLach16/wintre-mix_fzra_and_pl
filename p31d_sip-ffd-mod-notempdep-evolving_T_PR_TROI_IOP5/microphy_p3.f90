...
!__________________________________________________________________________________________!
 ...
 private
 public :: p3_main, polysvp1, p3_init, qv_sat
#ifdef ECCCGEM
...
! lookup table values for rain number- and mass-weighted fallspeeds and ventilation parameters
 ...
 real           :: rhosur,rhosui,ar,br,f1r,f2r,ecr,rhow,kr,kc,aimm,rin,mi0,trplpt,  &
...
!------------------------------------------------------------------------------------------!
 ...
 read_path = '/home/rml000/ords/p3/lookup_tables'            ! path for lookup tables from official model library
...
!==========================================================================================!
 ...
 SUBROUTINE p3_main(qc,nc,qr,nr,th_old,th,qv_old,qv,dt,qitot,qirim,qiliq,nitot,birim,     &
                    ...
                    freq3Ddiag_in,timer,timer_description,outfreq)
...
!----- Input/ouput arguments:  ----------------------------------------------------------!
 integer, intent(in),optional                         :: outfreq    !Output frequency for 1D model ML
...
!  all N process rates have units of # kg-1
 ...
 real, dimension(nCat) :: nrhetic    ! contact freezing with other ice rain (lachapelle et al. 2024)
...
! New process rates with log_LiquidFrac (present(qiliq))
 ...
 real, dimension(its:ite,kts:kte) :: i_dzq,i_rho,ze_ice,ze_rain,ze_cld,rho,rhofacr,      &
            rhofaci,xxls,xxlv,xlf,qvs,qvi,sup,supi,vtrmi1,tmparr1,i_exn,      &
            SCF,iSCF,SPF,iSPF,SPF_clr,Qv_cld,Qv_clr,prec,acn

 real, dimension(its:ite,kts:kte) :: massflux_r,nflux_r
 real, dimension(its:ite,kts:kte,nCat) :: massflux_i,nflux_i
 ...
 real  :: dum1_sip,dum2_sip,dum3_sip,dum4_sip,dum5_sip,dum6_sip,&
          mu_rcol,lamrcol,cdistrcol,logn0rcol,diam_col,d_ffd_thr ! variables added for SIP (lachapelle et al. 2024)
...
! added for triple moment ice
 ...
 real, dimension(its:ite,kts:kte)      :: qr_old,nr_old
 real, dimension(its:ite,kts:kte,nCat) :: qitot_old,nitot_old
  ...
  do i = its,ite
    ...
    !    (implying all mass is expected to evaporate/sublimate in one time step)
       ...
       if (qc(i,k).lt.qsmall .or. (qc(i,k).lt.qsmall_dry .and. sup(i,k).lt.-0.1)) then
          ...
          th(i,k) = th(i,k) !ML- i_exn(i,k)*qc(i,k)*xxlv(i,k)*i_cp
          qc(i,k) = 0.
          nc(i,k) = 0.
       endif
       ...
       if (qr(i,k).lt.qsmall .or. (qr(i,k).lt.qsmall_dry .and. sup(i,k).lt.-0.1)) then
          ...
          th(i,k) = th(i,k) !ML- i_exn(i,k)*qr(i,k)*xxlv(i,k)*i_cp
          qr(i,k) = 0.
          nr(i,k) = 0.
       endif
       ...
       do iice = 1,nCat
          if (qitot(i,k,iice).lt.qsmall .or. (qitot(i,k,iice).lt.qsmall_dry .and.        &
           supi(i,k).lt.-0.1)) then
             ...
             th(i,k) = th(i,k) !ML- i_exn(i,k)*(qitot(i,k,iice)-qiliq(i,k,iice))*           &
                                 ! xxls(i,k)*i_cp
             th(i,k) = th(i,k) !ML- i_exn(i,k)*qiliq(i,k,iice)*xxlv(i,k)*i_cp
             qitot(i,k,iice) = 0.
             nitot(i,k,iice) = 0.
             qirim(i,k,iice) = 0.
             qiliq(i,k,iice) = 0.
          ...
          if (log_LiquidFrac .and. qiliq(i,k,iice).ge.0.                                 &
             ...
             !freeze small amount of liquid (qiliq) to rime
                th(i,k) = th(i,k)!ML + i_exn(i,k)*qiliq(i,k,iice)*xlf(i,k)*i_cp
          ...
          if (qitot(i,k,iice).ge.qsmall .and. qitot(i,k,iice).lt.qsmall_dry              &
            ...
            !completely melt all tiny quantities of ice if T>0C
             qr(i,k) = qr(i,k) + qitot(i,k,iice)
             nr(i,k) = nr(i,k) + nitot(i,k,iice)
             th(i,k) = th(i,k) !ML- i_exn(i,k)*(qitot(i,k,iice)-qiliq(i,k,iice))*xlf(i,k)*  &
                                 ! i_cp
             qitot(i,k,iice) = 0.
             nitot(i,k,iice) = 0.
             qirim(i,k,iice) = 0.
...
!----------------------------------------------------------------------
        ...
        ! initialize inverse supersaturation relaxation timescale for combined ice categories
          ...
          iice_loop1: do iice = 1,nCat
             ...
             ! ice-rain collection processes
               ...
               ! if (k .eq. 95) then
               !    !  if ( (.not. (f1pr16 .gt. 1)) .and. (.not. (f1pr16 .lt. 2000.))) then
               !    print*, 'diam_ice : ', it, k,iice,f1pr16,diam_ice(i,k,iice),qitot(i,k,iice),nitot(i,k,iice)
               !    ! diam_ice(i,k,iice) = ((qitot(i,k,iice)*6.)/(nitot(i,k,iice)*500.*pi))**thrd
               !    ! print*, 'diam_ice corrected at : ', k,iice,f1pr16,diam_ice(i,k,iice)
               ! endif
               ! if (k .eq. 101) then
               !    print*, 'diam_ice : ', it, k,iice,f1pr16,diam_ice(i,k,iice),qitot(i,k,iice),nitot(i,k,iice)
               ! endif
   ...
   !.......................
             ...
             if (nitot(i,k,iice)>0.) then
               diam_ice(i,k,iice) = ((qitot(i,k,iice)*6.)/(nitot(i,k,iice)*900.*pi))**thrd
             endif
   ...
   !            rhorime_r(iice) = 400.
      ...
      ! nrcol is set equal to nr/dt and qrcol equal to qr/dt.Lachapelle 2023
      if ((sum(nrcol).gt.nsmall) .and. (sum(qrcol).gt.qsmall)) then
      if ( ( sum(nrcol)*dt.gt.nr(i,k)) .or. ( sum(qrcol)*dt.gt.qr(i,k)) ) then
      !   print*, 'Ok too much collection!'
        nrcol      = nrcol*nr(i,k)/sum(nrcol)/dt
        qrcol      = qrcol*qr(i,k)/sum(qrcol)/dt
      endif
      endif
   ...
   !         N_nuc = cons5*exp(log(cdistr(i,k))+log(gamma(mu_r(i,k)+4.))-3.*log(lamr(i,k)))*exp(aimm*(trplpt-t(i,k)))*SPF(i,k)
         ...
         ! if (nitot(i,k,iice_dest)>0.) then
         ! if (k .eq. 95) then
         !    print*, 'qrheti : ', it, k,iice_dest,diam_ice(i,k,iice_dest),qitot(i,k,iice_dest),nitot(i,k,iice_dest),qrheti(iice_dest)
         ! endif
         ! if (k .eq. 101) then
         !    print*, 'qrheti : ', it, k,iice_dest,diam_ice(i,k,iice_dest),qitot(i,k,iice_dest),nitot(i,k,iice_dest),qrheti(iice_dest)
         ! endif
   ...
   ! It only occurs in the temperature range of -8C < T -3C.
         ...
         if (.true.) then
          iice_loop_FFD:  do iice = 1,nCat
               ...
               calc_FFD:  if  ((qrcol(iice).gt.qsmall) .and. (t(i,k).gt.245.15) &
                  .and. (t(i,k).lt.270.15) ) then ! Fragmentation of freezing drops, as implemented in (lachapelle et al. 2025)

                  dum1_sip = qrcol(iice) + qrheti(iice)
                  dum2_sip = nrcol(iice) + nrheti(iice)
                  ...
                  dum6_sip = dum6_sip +  2.5e-11*(1.e24)*(d_ffd_thr)**4.*&
                              (10**logn0rcol)*( exp(-lamrcol*d_ffd_thr) )/lamrcol ! Add drops larger then d_ffd_thr
 
                  qrcol(iice) = qrcol(iice) - dum6_sip * (piov6*900.*(D_new)**3)      ! subtract splintering from rime mass transfer
                  if (qrcol(iice) .lt. 0. ) then
          ...
          if ( (t(i,k).lt.273.15) .and. (sum(qrcol).ge.qsmall) ) then
            ...
            do iice = 1,nCat
               ...
               if ( (diam_ice(i,k,iice)*2. .le. diam_col) .and. (qitot(i,k,iice).gt.qsmall) ) then  !ML
                  call icecat_destination(qitot(i,k,:)*iSCF(i,k),diam_ice(i,k,:), diam_col,&
                  deltaD_init,iice_dest)
...
!          endif
       ...
       iice_loop3: do iice = 1,nCat
        ...
        ! but accounts for rapid evaporation of small melting ice particles)
         ...
         ! endif
          ...
          nitot(i,k,iice) = nitot(i,k,iice) + (ninuc(iice)-nimlt(iice)-nisub(iice)-      &
                            ...
                            ncheti(iice)+nimul(iice)-nlevp(iice)+nrhetic(iice))*dt
        ...
        ! very small and the homogeneous temp. freezing threshold is approximate anyway.
          th(i,k) = th(i,k)!ML + i_exn(i,k)*((qidep(iice)-qisub(iice)+qinuc(iice))*         &
   ...
   !-- warm-phase only processes:
       ...
       th(i,k) = th(i,k) !ML+ i_exn(i,k)*((qcnuc+qccon+qrcon-qcevp-qrevp)*xxlv(i,k)*        &
               !   i_cp)*dt
...
!................................................................................
   ...
   IF (mod(it*dt,outfreq*60.)<1.e-5) THEN


      if (nCat.eq.1) then
      write(8000,'(1f35.9,1i5,20f50.9)') it*dt,k,qimlt(1)*10**8,qrmlt(1)*10**8,qrcol(1)*10**8,qccol(1)*10**8, &
            nrshdr(1)*10**8,nrcol(1),qisub(1)*10**8,qidep(1)*10**8, &
            qrcon*10**8,qrevp*10**8,qwgrth(1)*10**8,qinuc(1)*10**8, &
            qcheti(1)*10**8, qchetc(1)*10**8, qrhetc(1)*10**8, qrheti(1)*10**8, &
            qrmul(1)*10**8,qcshd(1)*10**8,nimul(1),diam_ice(i,k,1)

      write(9000,'(1f35.9,1i5,11f35.18)')  it*dt,k,ninuc(1),nimlt(1),nisub(1),&
            nislf(1),nrhetc(1),nrheti(1),nchetc(1),ncheti(1),nimul(1),nlevp(1),nrhetic(1)
      endif

      if (nCat.eq.2) then
      write(8000,'(1f35.9,1i5,39f50.9)') it*dt,k,qimlt(1)*10**8,qrmlt(1)*10**8,qrcol(1)*10**8,qccol(1)*10**8, &
            nrshdr(1)*10**8,nrcol(1),qisub(1)*10**8,qidep(1)*10**8, &
            qrcon*10**8,qrevp*10**8,qwgrth(1)*10**8,qinuc(1)*10**8, &
            qcheti(1)*10**8, qchetc(1)*10**8, qrhetc(1)*10**8, qrheti(1)*10**8, &
            qrmul(1)*10**8,qcshd(1)*10**8,nimul(1),diam_ice(i,k,1), &
            qimlt(2)*10**8,qrmlt(2)*10**8,qrcol(2)*10**8,qccol(2)*10**8, &
            nrshdr(2)*10**8,nrcol(2),qisub(2)*10**8,qidep(2)*10**8, &
            qwgrth(2)*10**8,qinuc(2)*10**8, &
            qcheti(2)*10**8, qchetc(2)*10**8, qrhetc(2)*10**8, qrheti(2)*10**8, &
            qrmul(2)*10**8,qcshd(2)*10**8,nimul(2),qicol(1,2)*10**8,diam_ice(i,k,2)

      write(9000,'(1f35.9,1i5,25f35.18)')  it*dt,k,ninuc(1),nimlt(1),nisub(1),&
            nislf(1),nrhetc(1),nrheti(1),nchetc(1),ncheti(1),nimul(1),nlevp(1),&
            nrhetic(1),nrslf,nrevp,ninuc(2),nimlt(2),nisub(2),&
            nislf(2),nrhetc(2),nrheti(2),nchetc(2),ncheti(2),nimul(2),nlevp(2),&
            nicol(1,2),nrhetic(2)
      endif

      if (nCat.eq.3) then
      write(8000,'(1f35.9,1i5,60f50.9)') it*dt,k,qimlt(1)*10**8,qrmlt(1)*10**8,qrcol(1)*10**8,qccol(1)*10**8, &
            nrshdr(1)*10**8,nrcol(1),qisub(1)*10**8,qidep(1)*10**8, &
            qrcon*10**8,qrevp*10**8,qwgrth(1)*10**8,qinuc(1)*10**8, &
            qcheti(1)*10**8, qchetc(1)*10**8, qrhetc(1)*10**8, qrheti(1)*10**8, &
            qrmul(1)*10**8,qcshd(1)*10**8,nimul(1),diam_ice(i,k,1), &
            ...
            qcheti(2)*10**8, qchetc(2)*10**8, qrhetc(2)*10**8, qrheti(2)*10**8, &
            qrmul(2)*10**8,qcshd(2)*10**8,nimul(2),qicol(1,2)*10**8,diam_ice(i,k,2), &
            qimlt(3)*10**8,qrmlt(3)*10**8,qrcol(3)*10**8,qccol(3)*10**8, &
            nrshdr(3)*10**8,nrcol(3),qisub(3)*10**8,qidep(3)*10**8, &
            qwgrth(3)*10**8,qinuc(3)*10**8, &
            qcheti(3)*10**8, qchetc(3)*10**8, qrhetc(3)*10**8, qrheti(3)*10**8, &
            qrmul(3)*10**8,qcshd(3)*10**8,nimul(3),qicol(1,3)*10**8,qicol(2,3)*10**8,diam_ice(i,k,3)

      write(9000,'(1f35.9,1i5,33f35.18)')  it*dt,k,ninuc(1),nimlt(1),nisub(1),&
            nislf(1),nrhetc(1),nrheti(1),nchetc(1),ncheti(1),nimul(1),nlevp(1),&
            ninuc(2),nimlt(2),nisub(2),&
            nislf(2),nrhetc(2),nrheti(2),nchetc(2),ncheti(2),nimul(2),nlevp(2),&
            nicol(1,2),&
            ninuc(3),nimlt(3),nisub(3),&
            nislf(3),nrhetc(3),nrheti(3),nchetc(3),ncheti(3),nimul(3),nlevp(3),&
            nicol(1,3),nicol(2,3)
      endif

      if (nCat.eq.4) then
      write(8000,'(1f35.9,1i5,77f50.9)') it*dt,k,qimlt(1)*10**8,qrmlt(1)*10**8,qrcol(1)*10**8,qccol(1)*10**8, &
            nrshdr(1)*10**8,nrcol(1),qisub(1)*10**8,qidep(1)*10**8, &
            qrcon*10**8,qrevp*10**8,qwgrth(1)*10**8,qinuc(1)*10**8, &
            qcheti(1)*10**8, qchetc(1)*10**8, qrhetc(1)*10**8, qrheti(1)*10**8, &
            qrmul(1)*10**8,qcshd(1)*10**8,nimul(1), &
            qimlt(2)*10**8,qrmlt(2)*10**8,qrcol(2)*10**8,qccol(2)*10**8, &
            nrshdr(2)*10**8,nrcol(2),qisub(2)*10**8,qidep(2)*10**8, &
            qwgrth(2)*10**8,qinuc(2)*10**8, &
            qcheti(2)*10**8, qchetc(2)*10**8, qrhetc(2)*10**8, qrheti(2)*10**8, &
            qrmul(2)*10**8,qcshd(2)*10**8,nimul(2),qicol(1,2)*10**8, &
            qimlt(3)*10**8,qrmlt(3)*10**8,qrcol(3)*10**8,qccol(3)*10**8, &
            nrshdr(3)*10**8,nrcol(3),qisub(3)*10**8,qidep(3)*10**8, &
            qwgrth(3)*10**8,qinuc(3)*10**8, &
            qcheti(3)*10**8, qchetc(3)*10**8, qrhetc(3)*10**8, qrheti(3)*10**8, &
            qrmul(3)*10**8,qcshd(3)*10**8,nimul(3),qicol(1,3)*10**8,qicol(2,3)*10**8, &
            qimlt(4)*10**8,qrmlt(4)*10**8,qrcol(4)*10**8,qccol(4)*10**8, &
            nrshdr(4)*10**8,nrcol(4),qisub(4)*10**8,qidep(4)*10**8, &
            qwgrth(4)*10**8,qinuc(4)*10**8, &
            qcheti(4)*10**8, qchetc(4)*10**8, qrhetc(4)*10**8, qrheti(4)*10**8, &
            qrmul(4)*10**8,qcshd(4)*10**8,nimul(4),qicol(1,4)*10**8,qicol(2,4)*10**8,qicol(3,4)*10**8,diam_ice(i,k,4)

      write(9000,'(1f35.9,1i5,46f35.18)')  it*dt,k,ninuc(1),nimlt(1),nisub(1),&
            nislf(1),nrhetc(1),nrheti(1),nchetc(1),ncheti(1),nimul(1),nlevp(1),&
            ninuc(2),nimlt(2),nisub(2),&
            nislf(2),nrhetc(2),nrheti(2),nchetc(2),ncheti(2),nimul(2),nlevp(2),&
            nicol(1,2),&
            ...
            nislf(3),nrhetc(3),nrheti(3),nchetc(3),ncheti(3),nimul(3),nlevp(3),&
            nicol(1,3),nicol(2,3),&
            ninuc(4),nimlt(4),nisub(4),&
            nislf(4),nrhetc(4),nrheti(4),nchetc(4),ncheti(4),nimul(4),nlevp(4),&
            nicol(1,4),nicol(2,4),nicol(3,4)
      endif
   ENDIF
...
! Rain:
    if (maxval(qr(i,:)) .ge. qsmall) then
       qr_old(i,:) =  qr(i,:)
       nr_old(i,:) =  nr(i,:)
       call sedimentation_liquid(qr(i,:),nr(i,:),2,iSPF(i,:),prt_liq(i),rho(i,:),        &
                       i_rho(i,:),i_dzq(i,:),dt,ktop,kbot,kdir,rhofacr=rhofacr(i,:),     &
                       massflux=massflux_r(i,:),nflux=nflux_r(i,:))
       diag_3d(i,:,5) =  qr(i,:) - qr_old(i,:)
       diag_3d(i,:,6) =  nr(i,:) - nr_old(i,:)
       
       diag_3d(i,:,11) =  massflux_r(i,:)
       diag_3d(i,:,12) =  nflux_r(i,:)
...
! Ice:
    ...
    if (log_3momentIce .and. log_LiquidFrac .and. log_tmp1) then
       call sedimentation_ice_TT(qitot(i,:,:),qirim(i,:,:),qiliq(i,:,:),nitot(i,:,:),    &
                              ...
                              kdir,dt,massflux=massflux_i(i,:,:),nflux=nflux_i(i,:,:))
    ...
    elseif (log_3momentIce .and. .not.log_LiquidFrac .and. log_tmp1) then
       call sedimentation_ice_TF(qitot(i,:,:),qirim(i,:,:),nitot(i,:,:),birim(i,:,:),    &
                              ...
                              massflux=massflux_i(i,:,:),nflux=nflux_i(i,:,:))
    ...
    elseif (.not. log_3momentIce .and. .not. log_LiquidFrac .and. log_tmp1) then
       ...
       call sedimentation_ice_FF(qitot(i,:,:),qirim(i,:,:),nitot(i,:,:),birim(i,:,:),    &
                              ...
                              massflux=massflux_i(i,:,:),nflux=nflux_i(i,:,:))
       diag_3d(i,:,1) =  qitot(i,:,1) - qitot_old(i,:,1)
       diag_3d(i,:,2) =  qitot(i,:,2) - qitot_old(i,:,2)
       diag_3d(i,:,3) =  nitot(i,:,1) - nitot_old(i,:,1)
       diag_3d(i,:,4) =  nitot(i,:,2) - nitot_old(i,:,2)
       
       diag_3d(i,:,7) = massflux_i(i,:,1)
       diag_3d(i,:,8) = massflux_i(i,:,2)
       diag_3d(i,:,9)  = nflux_i(i,:,1)
       diag_3d(i,:,10) = nflux_i(i,:,2)
    endif
    ...
    freezing_possible: if ( (qc(i,k)*iSCF(i,k).ge.qsmall .or. qr(i,k).ge.qsmall) .and.   &
       ...
       qc_not_small_2: if (qc(i,k)*iSCF(i,k).ge.qsmall .and. t(i,k).lt.233.15) then
         ...
         ! update theta. Note temperature is NOT updated here, but currently not used after
          th(i,k) = th(i,k) !ML+ i_exn(i,k)*Q_nuc*xlf(i,k)*i_cp
...
!------------------------------------------------------------------------------------
 ...
 !------------------
  ...
  qv_sat = ep_2*e_pres/max(1.e-3,(p_atm-0.*e_pres))
#else
...
!======================================================================================!
 subroutine sedimentation_liquid(qx,nx,liq_type,iSxF,prt_liq,rho,i_rho,i_dz,dt,ktop,     &
                                 kbot,kdir,acn,dnu,rhofacr,massflux,nflux)
...
!arguments:
 ...
 real, intent(out), dimension(:), optional :: massflux,nflux
 ...
 qx_present: if (log_qxpresent) then
    ...
    substep_sedi_r: do while (dt_left.gt.1.e-4)
      ...
      !RAIN only:
       ...
       if (present(nflux)) nflux = flux_nx
...
!======================================================================================!
 subroutine sedimentation_ice_TT(qit,qir,qil,nit,bir,zit,prt_sol,prt_soli,rho,i_rho,     &
                                 rhofac,i_dz,ktop,kbot,kdir,dt, massflux,nflux)
...
!              mflux_i(i,k) = flux_qit(k)  !store mass flux for use in visibility diagnostic)
          ...
          if (present(massflux)) massflux(:,iice) = flux_qit
          if (present(nflux))    nflux(:,iice)    = flux_nit
...
!======================================================================================!
 subroutine sedimentation_ice_TF(qit,qir,nit,bir,zit,prt_sol,prt_soli,rho,i_rho,rhofac,  &
                                 i_dz,ktop,kbot,kdir,dt, massflux,nflux)
